...
exports.run = async(client, message, args) => {
 if (!message.member.roles.cache.has("783639441839357962") && !message.member.hasPermission('KİCK_MEMBERS')) return message.channel.send(new Discord.MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setColor(0x00bfff).addField(`\`Bu Komutu Kullanmak içi Yeterli Yetkiniz Yok\``)).then(m => m.delete({timeout: 7000}));
  ...
  if(!uye || !crew) return message.channel.send(new Discord.MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setColor(0x00bfff).setDescription(`\`Geçerli bir üye ve sebep belirtmelisin!\``)).then(x => x.delete({timeout: 5000}));
  ...
  message.channel.send(new Discord.MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setColor(0x00bfff).setDescription(`${uye} üyesi, ${message.author} tarafından **${crew}** nedeniyle mutelendi!`)).catch();
  client.channels.cache.get("776784024558895114").send(new Discord.MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setColor(0x00bfff).setDescription(`${uye} İsimli Kullanıcı Metin Kanallarında Susturuldu.\n\n• Yetkili: <@!${message.author.id}> \`${message.author.id}\` \n• Mutelenen Üye: <@!${uye.id}> \`${uye.id}\` \n\n• Sebep: **${crew}**`)).catch();
};
exports.conf = {
    enabled: false,
    guildOnly: false,
