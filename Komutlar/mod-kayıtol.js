...
if(!sistem || sistem == null || sistem == undefined || sistem == 'kapat') return;
  ...
  if(!isim) return message.channel.send('Seni Kayıt Etmem İçin Bir İsim Girmelisin : `!!kayıtol Tolga 18`')
if(!yas) return message.channel.send('Yaş İçin En Az 1 Basamak Belirtebilirsin. Örnek: `!!kayıtol Tolga 18`')
  if(yas.length >= 3) return message.channel.send('Yaş İçin En Fazla 2 Basamak Belirtebilirsin. Örnek: `!!kayıtol Tolga 18`')
  if(isNaN(yas)) return message.channel.send('Yaş İçin Sayı Girmelisin. Örnek: `!!kayıtol Tolga 18`')
